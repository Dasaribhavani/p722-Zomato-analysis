...
#Q1.
UPDATE zomato_data SET Datekey_Opening = REPLACE(Datekey_Opening, '_', '/') WHERE Datekey_Opening LIKE '%_%';
alter table zomato_data modify column Datekey_Opening date;
select * from zomato_data;

select* from country_codes;
--------------------------------------------------------------------------------------------
#Q2.Calendar Table using the Column Datekey
...
-----------------------------------
select city, count(restaurantid) from zomato_data group by city;

------------------------------------------
#3.Find the Numbers of Resturants based on City and Country.

select country_codes.country_name,zomato_data.city,count(restaurantid)no_of_restaurants
from zomato_data inner join country_codes
on zomato_data.countrycode=country_codes.country_code
group by country_codes.country_name,zomato_data.city;
========================================
...
from zomato_data group by year(datekey_opening),quarter(datekey_opening),monthname(datekey_opening) 
...
select has_table_booking,concat(round(count(has_table_booking)/100,1),"%") percentage from zomato_data group by has_table_booking;

===============================================

# top 5 restaurants who has more number of votes
select  country_name,restaurantname,votes,Average_Cost_for_two from zomato_data inner join country_codes on zomato_data.countrycode=country_codes.country_code
group by country_codes.country_name,restaurantname,votes,Average_Cost_for_two
order by votes desc limit 5;

========================================
#Average rating per cusines
select cuisines, AVG(Rating) AS avg_rating
from zomato_data
group by cuisines
order by avg_rating DESC;
================================================
#Restaurants based on cuisines
